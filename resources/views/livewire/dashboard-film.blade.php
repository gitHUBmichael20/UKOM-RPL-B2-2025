<div class="pb-20 lg:pb-0">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            ...
            <div class="flex flex-wrap gap-3">
                <input type="text" wire:model.live.debounce.500ms="search" placeholder="Cari film atau sutradara..."
        ...
        <section class="mb-12">
            ...
            @if ($sedangTayang->count() > 0)
                <div class="relative group">
                            ...
                            <div wire:click="openModal({{ $film->id }})"
                                ...
                                <div
                                    ...
                                    <div class="flex justify-between items-center mt-2">
                                        ...
                                        <span
                                            class="px-2 py-0.5 text-xs font-bold rounded-full {{ $film->rating === 'SU'
                                                ? 'bg-green-500 text-white'
                                                : ($film->rating === 'R13+'
                                                    ? 'bg-yellow-500 text-white'
                                                    : ($film->rating === 'D17+'
                                                        ? 'bg-orange-500 text-white'
                                                        : 'bg-red-500 text-white')) }}">{{ $film->rating }}</span>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                        ...
                        @if ($hasMoreTayang)
                            <div wire:click="loadMoreTayang"
                                ...
                                <div class="text-center text-white p-4">
                                    ...
                                    <p class="text-xs opacity-90 mt-1">{{ $totalTayang - $sedangTayang->count() }} lagi
                                    </p>
                                </div>
                            </div>
                        @endif
                    ...
                    <!-- Scroll Buttons -->
                    <button
                        onclick="document.getElementById('tayang-container').scrollBy({left: -400, behavior: 'smooth'})"
        ...
        <section>
            ...
            @if ($segeraTayang->count() > 0)
                <div class="relative group">
                        ...
                        @foreach ($segeraTayang as $film)
                            <div wire:click="openModal({{ $film->id }})"
                                ...
                                <div class="absolute top-3 left-3 z-10">
                                    <span
                                        class="bg-purple-600 text-white px-2 py-1 rounded-full text-xs font-bold">Coming
                                        Soon</span>
                                </div>
                        ...
                        @if ($hasMoreSegera)
                            <div wire:click="loadMoreSegera"
                                ...
                                <div class="text-center text-white p-4">
                                    <i class="fa-solid fa-plus text-5xl mb-2"></i>
                                    <p class="font-bold text-sm">Lihat Lebih</p>
                                    <p class="text-xs opacity-90 mt-1">{{ $totalSegera - $segeraTayang->count() }} lagi
                                    </p>
                                </div>
                            </div>
                        @endif
                    ...
                    <button
                        onclick="document.getElementById('segera-container').scrollBy({left: -400, behavior: 'smooth'})"
                        class="hidden lg:flex absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 bg-white/90 hover:bg-white shadow-xl rounded-full w-12 h-12 items-center justify-center opacity-0 group-hover:opacity-100 transition z-10">
                        <i class="fa-solid fa-chevron-left text-xl"></i>
                    </button>
                    <button
    ...
    @if ($selectedFilm)
        <div class="fixed inset-0 bg-black/80 flex items-end lg:items-center justify-center z-50 p-0 lg:p-4 overflow-y-auto"
            ...
            <div class="lg:hidden absolute inset-0" wire:click="closeModal"></div>
            ...
            <div class="bg-white rounded-t-3xl lg:rounded-2xl w-full max-w-5xl max-h-[92vh] lg:max-h-[90vh] overflow-y-auto relative flex flex-col"
                wire:click.stop x-data="{ dragged: false }" @touchstart="dragged = false" @touchmove="dragged = true"
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm lg:text-base">
                                ...
                                <div>
                                    ...
                                    <span
                                        class="inline-block mt-1 px-3 py-1.5 rounded-full text-xs lg:text-sm font-bold
                                            {{ $selectedFilm->rating === 'SU'
                                                ? 'bg-green-100 text-green-800'
                                                : ($selectedFilm->rating === 'R13+'
                                                    ? 'bg-yellow-100 text-yellow-800'
                                                    : ($selectedFilm->rating === 'D17+'
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-3 mt-1">
                                        ...
                                        @php
                                            $fotoSutradara = $selectedFilm->sutradara?->foto_profil
                                                ? asset('storage/' . $selectedFilm->sutradara->foto_profil)
                                                : null;
                                        @endphp

                                        @if ($fotoSutradara)
                                            <img src="{{ $fotoSutradara }}"
                                                alt="{{ $selectedFilm->sutradara->nama_sutradara }}"
                                                class="w-10 h-10 rounded-full object-cover border-2 border-gray-200">
                                        @else
                                            <div
                                                class="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center border-2 border-gray-300">
                            ...
                            <div>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach ($selectedFilm->genres as $genre)
                                        <span
                                            class="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-full text-xs lg:text-sm">
                                            {{ $genre->nama_genre }}
                                        </span>
                                    @endforeach
                                </div>
